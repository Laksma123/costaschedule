...
async function parseWhatsAppPayload(text) {
  ...
  // ── 1. COMPRESSED QR FORMAT ([COSTA-PART n/t] or legacy CZ:n/t:base64) ──
  const czPattern = /(?:\[COSTA[- ]PART\s*|COSTA\s*|CZ:?)(\d+)\s*\/\s*(\d+)\]?:?\s*([\sA-Za-z0-9+/=]{10,})/gi;
  let match;
  let hasCZ = false;
  ...
  try {
    ...
    if (rawSaved) {
      ...
      if (parsed._savedAt && (Date.now() - parsed._savedAt > 30 * 60 * 1000)) {
